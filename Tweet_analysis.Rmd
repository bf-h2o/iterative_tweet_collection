...
plot_map_proj <- lat_list_dtp %>% filter(!is.na(lat)) %>%
...
# reload the data and collect in one list
list_of_tweet_file = read.csv("master_file-test_project-2020-06-15.csv", header = T) %>% select(x) 
