...
corpus_df <- corpus_df_init %>%  tm_map(content_transformer(tolower)) %>%
...
#Each plot can be treated independently. Example:
...
# Shouldn't remove the stopwords before making bigrams or risk spurious associations
my_TDM_bi <- TermDocumentMatrix(corpus_df_init, control = list(tokenize = tokenizer))
glimpse(my_TDM_bi)

# If the sparse terms are not removed before the transformation to matrix, the memory of the computer might be insufficient. keep a high sparse factor for bigram or most of it will be eliminated.
my_TDM_bi_r <- removeSparseTerms(my_TDM_bi, sparse = 0.998)

freq_bi <- rowSums(as.matrix(my_TDM_bi_r)) %>% 
        as.data.frame()  %>% 
        rownames_to_column() %>% 
        rename(c(freq_word = . , word = rowname))


# eliminate bigrams containing frequent words
freq_bi_sep <- freq_bi %>% separate(col = word,into = c("word_1","word_2"), sep = " ", remove = FALSE)

# stop_words contains frequent english words. 
freq_bi_clean <- freq_bi_sep %>% anti_join(stop_words, by = c("word_1" = "word")) %>% anti_join(stop_words, by = c("word_2" = "word")) 

freq_bi_clean %>% arrange(desc(freq_word)) %>% 
          slice(1:40) %>%
          ggplot(aes(x = reorder(word,freq_word), y = freq_word))+
          geom_point()+
          coord_flip()+
          theme_minimal()
...
```{r}
# search for bigrams associated with selected bigrams ("my_termes_bi")
my_termes_bi = c("black lives", "police brutality")

# find associated words. This can take a couple of minutes
word_assoc_bi <- findAssocs(my_TDM_bi_r, my_termes_bi, c(0.1, 0.1))
head(word_assoc_bi)

word_assoc_bi_vec <- list_vect2df(word_assoc_bi, col2 = "word", col3 = "assoc") %>% rename(term = X1)

plot_feq <-   word_assoc_bi_vec %>%  
              group_by(term) %>% 
              arrange(assoc, .by_group = T) %>% 
              slice(1:20) %>% 
              ggplot(aes(x = word, y = assoc, color = term))+
